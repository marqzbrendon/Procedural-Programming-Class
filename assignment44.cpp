...
*    Estimated:  0.5 hrs   
*    Actual:     1.0 hrs
*      I was having a problem with my for loop, which was causing me to
*      fail test 4. Troubleshooting the problem was the hardest part.
*      Other than that, undestanding the assignment was difficult.
************************************************************************/
...
float computeAverageLinear(int list[], int num)
{
   ...
   {
      sumLinear += linear(list, num, list[i]);
   }

   averageLinear = sumLinear / num;
...
float computeAverageBinary(int list[], int num)
{
   ...
   for (int i = 0; i < num && list[i]; i++)
   {
      sumBinary += binary(list, num, list[i]);
   }

   averageBinary = sumBinary / num;
